...
import './track_ByBatchId.html';

TreeData = new Mongo.Collection('TreeData');
var allBatches = new ReactiveArray(); 
...
Template['components_tree_view'].onRendered(function(){
	...
	if (result) {
		if(result.length >0){
			...
			{
				allBatches.push((result[i]));
			}
		}
	}
	return allBatches.list();
	});
})
...
Template['components_tree_view'].events({
    ...
    "click #btnTrackBatch": function(event, temp){ 
    var template = Template.instance();
    linearView = false;
    var batchID =temp.find("#batchID").value;
    Meteor.call('trackByBatchId',batchID,function(error, result){
        if (!error) {
            temp.find("#btnTrackBatch").disabled=false;
    ...
    treeArgs: {
         ...
         "events": { 
           "changed": function(e, item, data) {
			...
			//console.log(TreeData.find({_id:item[0]}).fetch());
            id=item[0]
            console.log(item[0])
    ...
    "populateDetails": function() {
        ...
        for (var k =0 ; k < treeDetails.length ;k++) {
            if(id===treeDetails[k]._id){
                ...
                var data = {
                    label1 : treeDetails[k].value.label1,
